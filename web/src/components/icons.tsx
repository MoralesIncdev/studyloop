...
export function Icon({
  ...
  style?: CSSProperties;
  /**
   * kimi P0 #5: icons default to decorative (`aria-hidden`) since almost
   * every call site is inside a button/element that already carries its own
   * `aria-label`. Pass this only for the rare icon that stands alone as the
   * sole content conveying meaning — it switches to `role="img"` with a
   * `<title>` instead of hiding from assistive tech.
   */
  ariaLabel?: string;
}): JSX.Element {
  ...
  return (
    <svg
      ...
      fill="currentColor"
      focusable="false"
      {...(accessible ? { role: "img", "aria-label": ariaLabel } : { "aria-hidden": "true" })}
    >
      {accessible && <title>{ariaLabel}</title>}
