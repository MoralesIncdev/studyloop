...
export const ReviewCardTransformSchema = z.object({
  ...
  why: z.string(),
  /**
   * V3-B review fix #2: source-content hash this transform was generated
   * from (see lib/cardTransform.ts's hashCardTransformInput) — optional so a
   * review.json written before this fix still parses instead of the whole
   * `transformed` map (and, via ReviewStateSchema, the whole file including
   * `cards` scheduling state) failing validation and rebuilding empty.
   * fillTransformCache treats a missing hash as invalid and drops/regenerates
   * the entry the next time it's loaded ("dropped on first load").
   */
...
export function deriveLiveCards(
   ...
   * (V3-B review finding #7): a pearl is model output, not learner-attested
   * or learner-generated by default — it only becomes reviewable once (a)
   * its `unitId` link exists AND that unit is feedable (attested, or has a
   * userTake), or (b) the learner explicitly added it via the rail's "Add to
   * review" action (lib/pearlReviewStore.ts), keyed by `pearlReviewKey(t)`.
   * Omitted (or empty) means "no pearls explicitly added", same as a
   * project with no pearlReviewAdds.json yet.
   */
  pearlReviewAdds: ReadonlySet<string> = new Set()
): ReviewCard[] {
  ...
  if (analysisVisible && analysis) {
    for (const p of analysis.pearls) {
      const linkedUnitFeedable = p.unitId !== undefined && isUnitFeedable(attestations[p.unitId]);
      const explicitlyAdded = pearlReviewAdds.has(pearlReviewKey(p.t));
      if (!linkedUnitFeedable && !explicitlyAdded) continue;
      cards.push({
 ...
 * - V3-B review finding #7 migration exception: a `pearl:` card that's no
 *   longer live SOLELY because it no longer meets the new pearl-gating rule
 *   (deriveLiveCards — no attested unit link, never explicitly added) but
 *   that the learner already actually reviewed at least once (`reps > 0`)
 *   is kept in state rather than dropped as an orphan — dropping it would
 *   silently erase real progress/mastery ("never delete learner progress"),
 *   not just clean up a deleted artifact. It simply stops resurfacing (it's
 *   not in `liveIds`, so it can never re-enter `dueCards` below) while still
 *   counting toward `masteryCount` if it graduated. A never-graded pearl
 *   (`reps === 0` — introduced by the old ungated behavior but never
 *   actually answered) has no progress to protect and is dropped normally.
...
export function buildReviewQueue(
  ...
  // Orphan dropping (with the pearl-gating migration exception above).
  ...
  for (const [id, cardState] of Object.entries(priorState.cards)) {
    ...
    if (id.startsWith("pearl:") && cardState.reps > 0) {
      cards[id] = cardState;
    }
  }
