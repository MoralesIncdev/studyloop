...
import { findActiveIndex, formatTimestamp, parseTimestamp } from "../src/lib/time.js";
...
describe("findActiveIndex", () => {
  ...
  it("finds the last item whose start is <= t", () => {
    ...
    expect(findActiveIndex(items, 5, (i) => i.start)).toBe(0);
    expect(findActiveIndex(items, 24, (i) => i.start)).toBe(1);
    expect(findActiveIndex(items, 100, (i) => i.start)).toBe(3);
  });
